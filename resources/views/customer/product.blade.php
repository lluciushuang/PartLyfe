    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <header class="h-20 glass-panel flex items-center justify-between px-8 flex-shrink-0 z-50 sticky top-0 border-b border-white/5 shadow-sm">
            <form action="{{ route('customer.dashboard') }}" method="GET" class="relative w-full max-w-3xl flex-grow">
                <input type="text" name="search" placeholder="Cari sparepart lain di Partlyfe..." class="w-full bg-slate-900/50 border border-white/10 rounded-xl py-2.5 pl-12 pr-6 focus:bg-slate-800 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-all text-sm text-white placeholder-slate-500">
            ...
            <div class="flex items-center gap-6 ml-8">
                ...
                @else
                    <a href="{{ route('login') }}" class="text-xs font-bold text-amber-400 bg-amber-500/10 px-4 py-2 rounded-full border border-amber-500/30 hover:bg-amber-500 hover:text-slate-900 transition">Masuk</a>
                @endauth
            </div>
        </header>
                ...
                <div class="flex gap-8 items-start">
                    ...
                    <div class="flex-grow min-w-0 pr-4">
                        ...
                        <div class="flex items-center gap-4 mb-6 text-sm">
                            ...
                            <span class="text-slate-600">|</span><p class="text-slate-400">Terjual <span class="font-bold text-white">1.2 rb+</span></p>
                        </div>
                        @php $retailPrice = $product->prices->where('price_level', 1)->first(); @endphp
                        <h2 class="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-rose-400 mb-8">Rp {{ number_format($retailPrice->price ?? 0, 0, ',', '.') }}</h2>
                        ...
                        <div class="glass-card rounded-2xl p-6 mb-8 border border-white/5">
                            ...
                            <div class="space-y-4 text-sm">
                                <div class="flex border-b border-white/5 pb-3">
                                    <p class="w-40 text-slate-400 font-medium">Merk</p><p class="font-bold text-amber-400">{{ $product->brand }}</p>
                                </div>
                                <div class="flex border-b border-white/5 pb-3">
                                    <p class="w-40 text-slate-400 font-medium">Kategori</p><p class="font-semibold text-white">{{ $product->category->name ?? 'Uncategorized' }}</p>
                                </div>
                                <div class="flex">
                                    <p class="w-40 text-slate-400 font-medium">SKU Part</p><p class="font-mono font-bold bg-white/10 px-2 rounded text-amber-300 text-xs py-0.5 border border-white/10">{{ $product->item_code }}</p>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="absolute bottom-4 right-4 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0 z-30">
                                    @auth
                                        <form action="{{ route('cart.add', $rec->id) }}" method="POST">
                                            @csrf
                                            <button type="submit" title="Tambah ke Keranjang" class="w-10 h-10 bg-amber-500 text-slate-900 font-black rounded-full flex items-center justify-center hover:bg-amber-400 shadow-[0_0_20px_rgba(245,158,11,0.5)] hover:scale-110 transition-transform">
                                                <i class="fa-solid fa-cart-plus text-sm"></i>
                                            </button>
                                        </form>
                                    @else
                                        <a href="{{ route('login') }}" class="w-10 h-10 bg-amber-500 text-slate-900 font-black rounded-full flex items-center justify-center hover:bg-amber-400 shadow-[0_0_20px_rgba(245,158,11,0.5)] hover:scale-110 transition-transform">
                                            <i class="fa-solid fa-cart-plus text-sm"></i>
                                        </a>
                                    @endauth
                                </div>
